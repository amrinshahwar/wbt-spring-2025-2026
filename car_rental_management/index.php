...
/* ------------- AJAX search endpoint -------------
   ...
   ?page=ajax&type=car&q=...        (librarian only)
*/
if ($page === 'ajax') {
    ...
    if ($type === 'staff' && $_SESSION['user']['role'] === 'admin') {
        echo json_encode($q === '' ? getStaffs($conn) : searchStaffs($conn, $q));
    } elseif ($type === 'car' && $_SESSION['user']['role'] === 'staff') {
        echo json_encode($q === '' ? getCars($conn) : searchCars($conn, $q));
    } else {
        http_response_code(403);
